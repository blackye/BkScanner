...
__all__ = ['MysqlCrackPlugin']

class MysqlCrackPlugin(PluginBase):

    name = "MysqlCrackPlugin"
    version = '1.0'
    ...
    def execute_run(self, ip, port, taskid):
        if str(port) == '3306':
            mysqlcrack = PortCrackBase(self.user_dict, self.pass_dict)
            self.threadpool.add_job(mysqlcrack.crack, ip = ip, port = port, service = self.service)
